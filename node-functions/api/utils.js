...
function getOpenai(context) {
    ...
    if (apiKey) {
        openai = new OpenAI({
            apiKey,
            baseURL: context.env.DASHSCOPE_BASE_URL || BASE_URL
        });
    }
    return openai;
}
...
export async function parseBody(context) {
...
export async function askQwenForJson(context, { prompt, images = [], textBlocks = [], temperature = 0.2, disableThinking = false }) {
