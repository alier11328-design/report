...
export default async function onRequest(context) {
    ...
    try {
        ...
        if (!productType || !materialType) {
            return createError('缺少产品类型或素材类型', 400);
        }

        const result = await askQwenForJson(context, {
            prompt: buildPosterCaptionPrompt({ productType, materialType, teacher, courseName, hasImages: images.length > 0 }),
            images,
            temperature: 0.8, disableThinking: true
        });
        const caption = normalizeString(result?.caption).trim();

        if (!caption) {
            return createError('AI 未返回文案，请重试', 502);
        }
